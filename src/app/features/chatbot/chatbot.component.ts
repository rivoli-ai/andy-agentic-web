...
interface ChatMessage {
  ...
  thinking?: string;
  /** Cached collapsed preview; updated on throttle ticks, not every change detection. */
...
export class ChatbotComponent implements OnInit, OnDestroy {
  ...
  /** Batches stream UI work — mermaid during stream freezes the tab; markdown alone is fine. */
  private streamUiRafId: number | null = null;
  private streamUiPendingWhileHidden = false;
  ...
  onDocumentVisibilityChange(): void {
    ...
    if (!this.documentHidden && this.streamUiPendingWhileHidden) {
      this.flushStreamUiUpdate();
    }
  }
            ...
            .map(msg => {
              const chatMessage: ChatMessage = {
                ...
                thinkingPreview: msg.thinking
                  ? this.computeThinkingPreview(msg.thinking, false)
                  : undefined,
  ...
  deleteSession(sessionId: string, event: Event): void {
    ...
    if (
      confirm('Are you sure you want to delete this chat session? This action cannot be undone.')
    ) {
      this.subscription.add(
  ...
  private executeAgent(userMessage: string, images?: ChatImage[]): void {
    ...
    this.currentStreamSubscription = this.chatService
      .sendMessageStream(
        userMessage,
  ...
  private scheduleStreamUiUpdate(): void {
    if (this.documentHidden) {
      this.streamUiPendingWhileHidden = true;
      return;
    }
    ...
    this.streamUiRafId = requestAnimationFrame(() => {
      this.streamUiRafId = null;
      this.streamUiPendingWhileHidden = false;
      this.cdr.detectChanges();
      this.scrollToBottom();
    });
  }

  private flushStreamUiUpdate(): void {
    if (this.streamUiRafId !== null) {
      cancelAnimationFrame(this.streamUiRafId);
      this.streamUiRafId = null;
    }
    ...
    this.streamUiPendingWhileHidden = false;
    const msg = this.currentStreamingMessage;
    if (msg?.thinking) {
      msg.thinkingPreview = this.computeThinkingPreview(msg.thinking, !!msg.isThinking);
    }
    this.cdr.detectChanges();
    this.scrollToBottom();
  }
  ...
  private updateLatestAgentMessageWithToolExecutions(history: ChatHistoryDto[]): void {
    ...
    if (!historyEntry) {
      historyEntry = history.find(
        h =>
          h.role === 'assistant' &&
          h.agentId === latestAgentMessage!.agentId &&
          Math.abs(new Date(h.timestamp).getTime() - latestAgentMessage!.timestamp.getTime()) <
    ...
    if (historyEntry && historyEntry.toolResults && historyEntry.toolResults.length > 0) {
      ...
      latestAgentMessage.toolExecutions = historyEntry.toolResults
        .map((toolLog: ToolExecutionLogDto) => this.convertToolExecutionLogToToolExecution(toolLog))
        .sort((a, b) => a.executedAt.getTime() - b.executedAt.getTime());
  ...
  /** Collapsed thinking preview: first line when done, live last line while streaming. */
  computeThinkingPreview(thinking: string | undefined, isThinking: boolean): string {
    if (!thinking) return '';
    ...
    const line = isThinking ? lines[lines.length - 1] : lines[0];
    ...
    if (imageItem) {
      ...
      if (file) {
        ...
        this.notificationService.success(
          'Image Pasted',
          'Image has been added from clipboard',
          3000
        );
      }
      return;
  ...
  isLongMessage(message: ChatMessage): boolean {
    ...
    return (
      message.content.length > this.MESSAGE_PREVIEW_LENGTH || lineCount > this.MESSAGE_PREVIEW_LINES
    );
  }
